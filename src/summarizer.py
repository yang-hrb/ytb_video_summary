...
"""
                ...
                prompt = f"""
你是一位专业的播客内容分析师和翻译专家。请对以下YouTube视频转录文本进行深度分析、纠正和总结。
...
1. **拼写和语音识别纠正**：
   - 仔细检查文本中的拼写错误、语音识别错误
   - 纠正所有错误的人名、地名、专业术语
   - 使用网络搜索验证不确定的术语和名称的正确写法
   - 将纠正后的内容自然地融入总结中

2. **内容翻译与总结**：
   - 将所有内容翻译成流畅、地道的简体中文
   - 不要限制总结的字数，根据内容的丰富程度自由发挥
   - 保留所有重要的细节、例子、数据和观点
   - 按照逻辑顺序组织内容，使用清晰的段落结构

3. **拼写和事实纠正**：
   - 识别文本中可能的拼写错误或语音识别错误
   - 如果涉及专业术语、人名、地名，请使用正确的写法
   - 对于不确定的内容，请标注[待确认]
...
1. 概括核心内容(5句以上)
2. 列出所有重要要点(5-10条,如果内容很多，可以适当增加)
3. 如果可能，创建时间线总结
4. 提供深入的分析和见解
